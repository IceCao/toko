...
class Worksheet implements IComparable
{
	...
	public function arrayToRow($source, $startColumn, $row)
	{
		foreach ($source as $cellValue) {
			if (is_array($cellValue)) {
				if ($cellValue[0] || $cellValue[0] == 0) {
					$this->getCell($startColumn . $row)->setValueExplicit($cellValue[0], $cellValue[1]);
				}
			} else {
				if ($cellValue || $cellValue == 0) {
					$this->getCell($startColumn . $row)->setValue($cellValue);
				}
			}
	...
	public function rowToArray($startColumn, $endColumn, $row)
	{
		$returnValue = [];
		while ($startColumn != $endColumn) {
			$returnValue[$startColumn] = $this->cellCollection->has($startColumn . $row) ?
			$this->cellCollection->get($startColumn . $row)->getValue() :
			null;
			++$startColumn;
		}
	...
	public function chunkToArray($startColumn, $endColumn, $startRow, $endRow)
	{
		$returnValue = [];
		while ($startRow != $endRow) {
			while ($startColumn != $endColumn) {
				$returnValue[][$startColumn] = $this->cellCollection->has($startColumn . $startRow) ?
				$this->cellCollection->get($startColumn . $startRow)->getValue() :
				null;
				++$startColumn;
			}
